...
use ANSR\Propel\Entity\Product;
use ANSR\Propel\Entity\ProductQuery;
...
class ProductTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'ANSR.Propel.Entity.Map.ProductTableMap';
     ...
     */
    const OM_CLASS = '\\ANSR\\Propel\\Entity\\Product';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'ANSR.Propel.Entity.Product';
     ...
     */
    const COL_TAX = 'products.tax';

    /**
     * the column name for the vendor_id field
     */
    const COL_VENDOR_ID = 'products.vendor_id';

    /**
     * the column name for the measure_id field
     */
    const COL_MEASURE_ID = 'products.measure_id';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('Id', 'Name', 'Tax', 'VendorId', 'MeasureId', ),
        self::TYPE_CAMELNAME     => array('id', 'name', 'tax', 'vendorId', 'measureId', ),
        self::TYPE_COLNAME       => array(ProductTableMap::COL_ID, ProductTableMap::COL_NAME, ProductTableMap::COL_TAX, ProductTableMap::COL_VENDOR_ID, ProductTableMap::COL_MEASURE_ID, ),
        self::TYPE_FIELDNAME     => array('id', 'name', 'tax', 'vendor_id', 'measure_id', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('Id' => 0, 'Name' => 1, 'Tax' => 2, 'VendorId' => 3, 'MeasureId' => 4, ),
        self::TYPE_CAMELNAME     => array('id' => 0, 'name' => 1, 'tax' => 2, 'vendorId' => 3, 'measureId' => 4, ),
        self::TYPE_COLNAME       => array(ProductTableMap::COL_ID => 0, ProductTableMap::COL_NAME => 1, ProductTableMap::COL_TAX => 2, ProductTableMap::COL_VENDOR_ID => 3, ProductTableMap::COL_MEASURE_ID => 4, ),
        self::TYPE_FIELDNAME     => array('id' => 0, 'name' => 1, 'tax' => 2, 'vendor_id' => 3, 'measure_id' => 4, ),
        self::TYPE_NUM           => array(0, 1, 2, 3, 4, )
    );
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\ANSR\\Propel\\Entity\\Product');
        $this->setPackage('ANSR.Propel.Entity');
        $this->setUseIdGenerator(true);
        // columns
        $this->addPrimaryKey('id', 'Id', 'INTEGER', true, null, null);
        $this->addColumn('name', 'Name', 'VARCHAR', true, 128, null);
        $this->addColumn('tax', 'Tax', 'INTEGER', true, null, null);
        $this->addForeignKey('vendor_id', 'VendorId', 'INTEGER', 'vendors', 'id', true, null, null);
        $this->addForeignKey('measure_id', 'MeasureId', 'INTEGER', 'measures', 'id', true, null, null);
    } // initialize()
  ...
  array (
    ...
    1 => ':id',
        ...
        $this->addRelation('Measure', '\\ANSR\\Propel\\Entity\\Measure', RelationMap::MANY_TO_ONE, array (
  0 =>
  array (
    0 => ':measure_id',
    1 => ':id',
  ...
  array (
    ...
    public static function addSelectColumns(Criteria $criteria, $alias = null)
    {
        if (null === $alias) {
            ...
            $criteria->addSelectColumn(ProductTableMap::COL_TAX);
            $criteria->addSelectColumn(ProductTableMap::COL_VENDOR_ID);
            $criteria->addSelectColumn(ProductTableMap::COL_MEASURE_ID);
        } else {
            $criteria->addSelectColumn($alias . '.id');
            $criteria->addSelectColumn($alias . '.name');
            $criteria->addSelectColumn($alias . '.tax');
            $criteria->addSelectColumn($alias . '.vendor_id');
            $criteria->addSelectColumn($alias . '.measure_id');
        }
    }
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \ANSR\Propel\Entity\Product) { // it's a model object
